...
# - Các lần sau: tự phát hiện proxy cũ, in lại thông tin + firewall status
...
#--------------------------------------------------
# 1. Hàm kiểm tra / tạo firewall rule gcp-proxy-ports
#    -> luôn gọi cả ở lần đầu và lần sau
#--------------------------------------------------
create_or_check_firewall_rule() {
  echo
...
#--------------------------------------------------
# 2. Nếu đã có proxy_info.txt -> chỉ restart service, check firewall, in lại proxy
#--------------------------------------------------
if [ -f "$INFO_FILE" ]; then
  echo
  ...
  # Luôn kiểm tra firewall, có thì báo "exists", chưa có thì tạo
  create_or_check_firewall_rule
  print_firewall_summary
...
#--------------------------------------------------
# 3. Nếu chưa có proxy_info.txt -> lần cài đặt đầu tiên
#--------------------------------------------------
echo "=== 3proxy auto proxy installer (first-time setup) ==="

# 3.1 Cài package cần thiết
apt-get update -y
apt-get install -y build-essential curl wget openssl

# 3.2 Tải & build 3proxy (nếu chưa có)
...
# 3.5 Kiểm tra / tạo firewall rule
create_or_check_firewall_rule

# 3.6 Lấy IP public & in ra proxy + lưu vào /root/proxy_info.txt
METADATA_HEADER="Metadata-Flavor: Google"
